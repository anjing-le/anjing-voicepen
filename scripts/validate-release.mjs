...
export const PLATFORM_KEYS = ["darwin-aarch64", "darwin-x86_64", "windows-x86_64"];
...
export function parseCargoPackageVersion(raw) {
  ...
  throw new ValidationError([issue("src-tauri/Cargo.toml package.version", "is missing")]);
}
...
export function validateVersions({ packageVersion, cargoVersion, tauriVersion }) {
  const issues = [];
  validateStableVersion(packageVersion, "package.json.version", issues);
  validateStableVersion(cargoVersion, "src-tauri/Cargo.toml package.version", issues);
  validateStableVersion(tauriVersion, "src-tauri/tauri.conf.json.version", issues);
  if (new Set([packageVersion, cargoVersion, tauriVersion]).size !== 1) {
    issues.push(issue("version", "package.json, Cargo.toml, and tauri.conf.json must match"));
  }
...
export function validateTag(tag, version) {
  if (!tag) throw new ValidationError([issue("tag", "is required via --tag or GITHUB_REF_NAME")]);
  if (tag !== `v${version}`) throw new ValidationError([issue("tag", `must equal v${version}`)]);
}
...
function validateAssetUrl(value, platform, tag, issues) {
  ...
  if (url.port || url.username || url.password || url.search || url.hash) {
    issues.push(issue(field, "must not include a port, credentials, query parameters, or fragments"));
  }
  const expectedPrefix = `${RELEASE_PATH_PREFIX}${tag}/`;
  if (!url.pathname.startsWith(expectedPrefix)) {
    issues.push(issue(field, `must reference the immutable ${tag} release in anjing-le/anjing-voicepen`));
    return value;
  }
  ...
  } else if (!asset.toLocaleLowerCase("en-US").endsWith(ASSET_SUFFIXES[platform])) {
    issues.push(issue(field, `must reference a ${ASSET_SUFFIXES[platform]} updater asset`));
  }
  return value;
}

export function validateManifest(manifest, { version, tag }) {
  const issues = [];
  if (!manifest || typeof manifest !== "object" || Array.isArray(manifest)) {
    throw new ValidationError([issue("latest.json", "must be a JSON object")]);
  }
  if (manifest.version !== version) {
    issues.push(issue("latest.json.version", `must equal ${version} without a v prefix`));
  }
  if (typeof manifest.notes !== "string" || manifest.notes.trim() === "") {
    ...
    if ([...manifest.notes].length > MAX_NOTES_CHARACTERS) {
      issues.push(issue("latest.json.notes", `must not exceed ${MAX_NOTES_CHARACTERS} Unicode characters`));
    }
    if (DISALLOWED_NOTES_CONTROL.test(manifest.notes)) {
      issues.push(issue("latest.json.notes", "must not contain control characters other than tab or newline"));
    }
  }
  if (
    ...
    issues.push(issue("latest.json.pub_date", "must be a valid ISO/RFC3339 timestamp"));
  }
  ...
  if (!platforms || typeof platforms !== "object" || Array.isArray(platforms)) {
    ...
    for (const platform of keys) {
      if (!PLATFORM_KEYS.includes(platform)) {
        issues.push(issue(`platforms.${platform}`, "is not in the supported release matrix"));
      }
    }
    const urls = new Map();
    for (const platform of PLATFORM_KEYS) {
      ...
      if (url) {
        if (urls.has(url)) {
          issues.push(issue(`platforms.${platform}.url`, `duplicates ${urls.get(url)}`));
        } else {
      ...
      if (typeof signature !== "string" || signature.trim() === "") {
        issues.push(issue(`platforms.${platform}.signature`, "must be a non-empty inline signature"));
      } else if (/^https?:\/\//iu.test(signature.trim())) {
        issues.push(issue(`platforms.${platform}.signature`, "must be inline data, not a URL"));
      }
    }
  }
  if (issues.length) throw new ValidationError(issues);
...
export async function validateRepository({ root, manifestPath, tag, versionsOnly = false }) {
  ...
  const manifestRaw = await readFile(path.resolve(root, manifestPath ?? "latest.json"), "utf8");
  validateManifest(parseJson(manifestRaw, "latest.json"), { version, tag });
  return version;
}

function parseArguments(argv) {
  const options = { root: process.cwd(), manifestPath: "latest.json", tag: process.env.GITHUB_REF_NAME };
  for (let index = 0; index < argv.length; index += 1) {
    const argument = argv[index];
    if (argument === "--versions-only") options.versionsOnly = true;
    else if (argument === "--root" || argument === "--manifest" || argument === "--tag") {
...
if (isMain) {
  try {
    ...
    console.log(options.versionsOnly ? `versions valid: ${version}` : `release metadata valid: v${version}`);
  } catch (error) {
