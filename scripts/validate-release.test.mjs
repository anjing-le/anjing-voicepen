...
test("macos scope requires exactly the two signed macOS updater entries", () => {
  const macosManifest = manifest();
  delete macosManifest.platforms["windows-x86_64"];
  assert.doesNotThrow(() =>
    validateManifest(macosManifest, { version, tag, scope: "macos" }),
  );

  delete macosManifest.platforms["darwin-x86_64"];
  assert.ok(
    issuesFor(() =>
      validateManifest(macosManifest, { version, tag, scope: "macos" }),
    ).some((entry) => entry.includes("darwin-x86_64")),
  );

  const unexpectedWindows = manifest();
  assert.ok(
    issuesFor(() =>
      validateManifest(unexpectedWindows, { version, tag, scope: "macos" }),
    ).some((entry) => entry.includes("windows-x86_64")),
  );
});

test("all remains the default scope and invalid scopes are rejected", () => {
  const missingWindows = manifest();
  delete missingWindows.platforms["windows-x86_64"];
  assert.ok(
    issuesFor(() => validateManifest(missingWindows, { version, tag })).some(
      (entry) => entry.includes("windows-x86_64"),
    ),
  );
  assert.ok(
    issuesFor(() =>
      validateManifest(manifest(), { version, tag, scope: "linux" }),
    ...
    assert.ok(
      issuesFor(() =>
        validateVersions({
          ...
          cargoVersion: invalid,
          tauriVersion: invalid,
        }),
...
test("Cargo parser reads package version rather than dependency versions", () => {
  ...
  assert.throws(() =>
    parseCargoPackageVersion('[dependencies]\nfoo = { version = "1" }'),
  );
});
  ...
  for (const manifestVersion of ["1.2.4", "v1.2.3"]) {
    ...
    assert.ok(
      issuesFor(() => validateManifest(value, { version, tag })).some((entry) =>
        entry.includes("latest.json.version"),
      ),
    );
  }
});
...
test("requires a tag that exactly matches the stable version", () => {
  for (const invalid of [undefined, "", "1.2.3", "v1.2.4", "v1.2.3-rc.1"]) {
    assert.ok(
      issuesFor(() => validateTag(invalid, version)).some((entry) =>
...
test("rejects missing, extra, and misspelled platform keys", () => {
  ...
  assert.ok(
    issuesFor(() => validateManifest(missing, { version, tag })).some((entry) =>
      entry.includes("darwin-aarch64"),
    ),
  );
  ...
  for (const key of ["linux-x86_64", "windows-aarch64", "darwin-arm64"]) {
    ...
    assert.ok(
      issuesFor(() => validateManifest(extra, { version, tag })).some((entry) =>
        entry.includes(key),
      ),
    );
  }
...
test("rejects mutable, cross-version, cross-repository, and unsafe URLs", () => {
  ...
  for (const url of cases) {
    ...
    assert.ok(
      issuesFor(() => validateManifest(value, { version, tag })).some((entry) =>
        entry.includes("darwin-aarch64.url"),
      ),
    );
  }
});
...
test("rejects empty, nested, encoded-slash, and duplicate asset URLs", () => {
  for (const suffix of [
    "",
    "nested/app.app.tar.gz",
    "nested%2Fapp.app.tar.gz",
    ".",
    "..",
  ]) {
    const value = manifest();
    value.platforms["darwin-aarch64"].url =
      `https://github.com/anjing-le/anjing-voicepen/releases/download/${tag}/${suffix}`;
    assert.ok(
      issuesFor(() => validateManifest(value, { version, tag })).some((entry) =>
        entry.includes("darwin-aarch64.url"),
      ),
  ...
  duplicate.platforms["windows-x86_64"].url =
    duplicate.platforms["darwin-x86_64"].url;
  assert.ok(
    issuesFor(() => validateManifest(duplicate, { version, tag })).some(
      (entry) => entry.includes("duplicates"),
    ),
  );
});
...
test("enforces updater asset types for every platform key", () => {
  ...
  for (const [platform, filename] of Object.entries(wrongExtensions)) {
    const value = manifest();
    value.platforms[platform].url =
      `https://github.com/anjing-le/anjing-voicepen/releases/download/${tag}/${filename}`;
    assert.ok(
      issuesFor(() => validateManifest(value, { version, tag })).some((entry) =>
  ...
  assert.doesNotThrow(() =>
    validateManifest(caseInsensitive, { version, tag }),
  );
});
...
test("requires inline non-empty signatures", () => {
  for (const signature of [
    "",
    "   ",
    "https://github.com/signature.sig",
    "HTTP://example.com/sig",
  ]) {
    const value = manifest();
...
test("requires user-visible notes and a valid publication date", () => {
  ...
  noNotes.notes = " ";
  assert.ok(
    issuesFor(() => validateManifest(noNotes, { version, tag })).some((entry) =>
      entry.includes("notes"),
    ),
  );
  const badDate = manifest();
  badDate.pub_date = "not-a-date";
  assert.ok(
    issuesFor(() => validateManifest(badDate, { version, tag })).some((entry) =>
      entry.includes("pub_date"),
    ),
  );
  const looseDate = manifest();
  looseDate.pub_date = "2026-08-05";
  assert.ok(
    issuesFor(() => validateManifest(looseDate, { version, tag })).some(
      (entry) => entry.includes("pub_date"),
    ),
  );
});
...
test("limits notes to safe plain text accepted by the application", () => {
  ...
  assert.ok(
    issuesFor(() => validateManifest(tooLong, { version, tag })).some((entry) =>
      entry.includes("8000"),
    ),
  );
