...
- VoicePen 版本、commit 与安装方式（dev / macOS ad-hoc 未公证 / Windows signed）
- OS 名称与完整版本、CPU 架构、设备类型
- 测试日期、执行人、操作步骤和证据链接
- 失败时的实际表现、恢复方式和关联 Issue

当前仓库尚无按既定分发方式完成的双端验收证据，因此以下目标平台均从 `未测` 开始。
...
| 平台 | 架构 | 当前状态 | 发布验收要求 |
| --- | --- | --- | --- |
| macOS | Apple Silicon | 未测 | ad-hoc、未公证安装；系统放行、权限、核心闭环、升级与配置保留 |
| macOS | Intel | 未测 | ad-hoc、未公证安装；系统放行、权限、核心闭环、升级与配置保留 |
...
- macOS：检查 Privacy & Security → Microphone 中的应用身份；开发版结果不能替代 Release 的 ad-hoc `.app`。
...
正式发布前分别完成：干净账户安装、Gatekeeper/SmartScreen 表现、首次权限、卸载、旧版到新版 OTA、取消更新、网络失败、Updater 签名失败、配置保留和故障恢复。macOS 必须记录首次及 OTA 后是否再次需要“仍要打开”和权限重授。只有三个目标平台按既定分发方式的安装与核心闭环证据齐全，才能称为双端正式支持。
