...
5. 所有可能改变系统状态的操作先说明影响。正常安装、授权和卸载由我在确认后执行；不要绕过 Gatekeeper、SmartScreen、代码签名或系统权限保护，不要关闭安全软件。
...
- 检查下载来源、macOS Developer ID/Gatekeeper 或 Windows Authenticode/SmartScreen，再完成安装和首次启动。
...
- 如果 B 已正式发布：先确认 A 的“稍后更新”，再执行 A → B OTA，核对更新要点、用户确认、升级后版本、配置保留和核心闭环。
