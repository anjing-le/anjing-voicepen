...
| TD-005 | 已有核心单元测试，但尚无 CI 与完整平台/集成测试 | Technical Debt | 配置、Provider、Runtime 当前共有 22 个 Rust 单元测试；仓库仍无 GitHub Actions workflow | 无 | 直接 push `main` 时测试不会自动拦截；真实平台回归仍依赖人工 | 保留 `npm run build`、Rust test/check/clippy 本地门禁 | 扩充协调与平台测试，并让 main push 自动执行 | CI 对每次 push 执行前端构建、Rust 检查/测试并稳定通过 | Repayment in Progress |
