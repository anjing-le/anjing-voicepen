...
| TD-004 | 双端能力尚无真实安装/权限证据 | Unknown Cause / Unverified Capability | 已建立 `PLATFORM_ACCEPTANCE.md`，但所有目标仍为未测；仓库无正式 Release | Windows 与 Intel Mac 是否运行过未知 | 编译成功被误报为产品支持；权限和粘贴可能失败 | 目标矩阵为 macOS arm64/x64、Windows x64 | macOS 按 ad-hoc 未公证流程、Windows 按可信签名流程积累证据 | 三个目标均有安装、录音、快捷键、粘贴、OTA 的可追溯证据 | Repayment in Progress |
| TD-005 | 双系统 CI 已建立并完成首次远端运行，完整平台/集成测试仍依赖真机 | Technical Debt | 当前共有 34 个 Rust 单元测试；`ci.yml` 在 macOS、Windows 执行前端构建、版本校验、Rust fmt/test/check/clippy；修复 Windows Clippy 后 Actions run `30991009911` 双端成功 | 长期稳定性和真实平台行为仍需后续运行证明 | CI 可发现编译与静态问题，但不能覆盖系统权限、设备和安装行为 | 保留本地等价门禁与双系统 CI，真实行为进入 Stage 5 验收 | 持续观察 main/PR CI；按干净电脑协议补真实平台证据 | CI 持续稳定，关键双端行为另有真机证据 | Repayment in Progress |
| TD-006 | 已有 OTA 应用内流程，但尚无真实 Release、跨版本或回滚验证 | Technical Debt | Rust Updater 使用固定 GitHub endpoint、编译期公钥、用户确认和纯文本 notes；macOS 决策为 ad-hoc 未公证，尚无 Release/真机升级证据 | OTA 后 Gatekeeper 与 TCC 权限表现未知 | 仅凭本地 UI/测试不能证明真实升级安全或可用性 | GitHub Releases 单通道、用户确认更新、minisign 信任链 | 未注入公钥的构建在联网前停止；macOS 不伪装 Apple 已验证；正式发布需单独批准 | 三平台产物按矩阵构建并验签；旧版升级、系统放行、权限与失败保留验证完成 | Repayment in Progress |
...
| TD-012 | `macOSPrivateApi` 已启用但必要性和长期影响未记录 | Unknown Cause | Tauri 配置和依赖 feature 均启用该选项 | 哪个窗口行为必须依赖私有 API 未知 | 系统兼容性或未来升级带来不明确风险 | 当前无边框透明浮窗行为 | 在改变前做只读追踪与平台验证 | 明确依赖点和理由后登记保留，或验证移除不影响体验 | Needs Investigation |
| TD-017 | macOS 明确采用 ad-hoc code seal 且不做 Apple 公证 | Intentional Design / External Constraint | 用户于 2026-08-05 明确决定“不走公证”；Tauri 配置固定 `signingIdentity: "-"`，workflow 拒绝 Developer ID Authority 并保留 updater minisign | OTA 替换后 Gatekeeper 例外和 TCC 权限是否沿用必须真机证明 | 首次打开必有额外系统放行摩擦；Apple 未验证发布者或扫描恶意软件，用户可能误解信任级别 | 只从 `anjing-le/anjing-voicepen` GitHub Releases 分发；不使用脚本绕过 Gatekeeper | 文档明确风险与系统 UI 放行路径；A→B 验收记录再次放行和权限变化 | 两个 Mac 架构验收项有真实证据；若未来改用 Developer ID，则另立迁移决策与门禁 | Confirmed to Preserve |
| TD-013 | WebView 已启用最小 CSP，但尚缺 macOS/Windows 打包 WebView 的真实回归 | Technical Debt | CSP 仅允许本地资源与 Tauri IPC；Provider 和 updater 网络均由 Rust 发起 | 两个平台 WebView 对生产 CSP 的细节差异尚未实测 | CSP 配置错误可能使设置或浮窗在特定系统失效 | 现有本地 UI、IPC 与 Rust 网络边界保持可用 | 自动构建验证配置，Stage 5 在正式发布产物中回归全部 UI/IPC | 三目标发布产物的设置、浮窗、Provider 与 OTA 均通过 CSP 下真机验证 | Repayment in Progress |
...
| TD-016 | Linux GTK 依赖链锁定 `glib 0.18`，受 GHSA-wrw7-89jp-8q8g 影响 | External Constraint | `cargo tree --target` 证明 macOS arm64/x64 与 Windows x64 均不包含 glib；它只由非正式支持的 Linux GTK/WebKit 链引入。当前 Tauri 2.11.x 仍依赖 GTK 0.18，直接加入 glib 0.20 只会双版本共存 | Tauri 何时升级 GTK/glib 主版本未知 | GitHub 仍显示中危告警；若未来分发 Linux 构建则需重新评估实际可达性 | 当前正式发布矩阵仅 macOS/Windows，Linux 保持不承诺、不分发 | 不忽略或伪报已修复；持续跟踪 Tauri 上游，在任何 Linux 支持决定前阻断发布并复审 | 上游 Tauri/GTK 支持 glib 0.20+，升级后告警关闭且双端回归通过 | Awaiting Historical Context |
