...
| TD-005 | 已建立双系统 CI 代码，但尚无首次远端运行与完整平台/集成测试证据 | Technical Debt | 当前共有 34 个 Rust 单元测试；`ci.yml` 为 main/PR 在 macOS、Windows 执行前端构建、版本校验、Rust fmt/test/check/clippy | 新 workflow 尚未由本提交后的远端运行证明 | workflow 或 runner 差异可能导致 main 检查失败；真实平台回归仍依赖人工 | 保留本地等价门禁，并观察首次远端 Actions 结果 | CI 代码已落地，推送后核对两系统首次运行；后续扩充真实平台回归 | main/PR 的 macOS、Windows CI 稳定通过，关键双端行为另有真机证据 | Repayment in Progress |
